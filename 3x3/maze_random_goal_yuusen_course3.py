...
# 赤い壁を描く
plt.plot([1, 1], [3, 2], color='red', linewidth=2)
plt.plot([1, 2], [1, 1], color='red', linewidth=2)
plt.plot([2, 2], [0, 2], color='red', linewidth=2)
...
# 行は状態0～7、列は移動方向で↑、→、↓、←を表す
theta_0 = np.array([[np.nan, np.nan, 1, np.nan],  # s0
                    [np.nan, 1, 1, np.nan],  # s1
                    [np.nan, np.nan, 1, 1],  # s2
                    [1, 1, 1, np.nan],  # s3
                    [1, np.nan, np.nan, 1],  # s4
                    [1, np.nan, 1, np.nan],  # s5
                    [1, 1, np.nan, np.nan],  # s6
                    [np.nan, np.nan, np.nan, 1],  # s7、※s8はゴールなので、方策はなし
                    ])
...
def get_next_s(pi, s):
    ...
    #if pi[s,1]!=0 or pi[s,2]!=0:
    #    print("右に行く:{}".format(pi[s,1]))
    #    print("下に行く:{}".format(pi[s,2]))
    
    
    

    next_direction = np.random.choice(direction, p=pi[s, :]) #現在位置での遷移確率で方向を選択
