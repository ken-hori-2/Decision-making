...
def AAA(s):
    ...
    if AA[s][1]==1:
        if A[s+1] < 2:
            ...
            for i in range(0, m):
                for j in range(0, n):
                    if not(np.isnan(theta[i, j])):  # thetaがnanでない場合
                        ...
                        #SA_i = [SA for SA in state_history if SA[0] == i]
                        # # 履歴から状態iのものを取り出すリスト内包表記です

                        #SA_ij = [SA for SA in state_history if SA == [i, j]]
                        # # 状態iで行動jをしたものを取り出す
                        ...
                        # # 初版では符号の正負に間違いがありました（修正日：180703）
                        # #delta_theta[i, j] = (N_ij + pi[i, j] * N_i) / T
                        #delta_theta[i, j] = (1 - pi_0[i, j] * 1) / T
                        if pi_0[i,1]>0.1:
                            pi_0[i,1]=pi_0[i,1]-0.1
                            print('pi_0[{},1]={}'.format(i,pi_0[i,1]))
            ...
            print('exp_theta={}'.format(exp_theta))
