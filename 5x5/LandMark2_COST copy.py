...
#lt.plot([4, 5], [3, 3], color='black', linewidth=2)
plt.plot([4, 6], [1, 1], color='black', linewidth=2)
plt.plot([3, 3], [0, 1], color='black', linewidth=2)
plt.plot([2, 2], [1, 2], color='black', linewidth=2)


plt.plot([4, 4], [1, 3], color='black', linewidth=2)
...
plt.text(2.5, 3.3, 'START', ha='center')
plt.text(0.5, 5.3, 'GOAL', ha='center')
...
def AAA(s,depth):
    ...
    if depth > 3 and LandMark  == False: #4:
        #excp = Exception()
        #excp.value = state_history
        #raise excp
        A[s] = 0 #1 #0
...
def animate(i):
    ...
    ax.plot([4.0], [1.5], marker="d", color='b', markersize=20)

    X2[i] = x2  #-0.2
    Y2[i] = y2-0.3

    #TXT = ax.text(1.5, 6.3, "cost:" + str(i), fontsize=20)

    #line2.set_data(X[:i]+1.0, Y[:i]-0.
    #TXT = ax.text(x2, y2, "cost:{}".format(i), fontsize=8)
    #TXT.set_txt(X2[i], Y2[i])


    #TXT = ax.text(1.5, 6.3, "reward/sec:{}" .format(i), fontsize=20)
    #ax.text(1.5, 6.3, "reward/sec:{}" .format(i), fontsize=20)

    #ax.text(1.5, 6.3, 'delta={:.2f}'.format(i),
    #                ha='center', va='bottom',
